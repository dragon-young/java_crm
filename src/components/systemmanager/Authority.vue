    ...
    <div>
        ...
        <el-card class="box-card">
            <div slot="header" class="clearfix">
                <span>权限管理</span>
                <el-button style="float: right; padding: 3px 0" type="text" @click="reload">重新加载</el-button>
            </div>
             <el-table
                :data="pageNum.data"
                stripe
                ...
                label="操作">
                    <template slot-scope="scope">
                        <el-button icon="el-icon-delete" type="danger" size="mini" @click="deleteAuthority(scope.row.id)">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
export default {
  ...
  methods: {
    ...
    async deleteAuthority (id) {
      console.log(id)
      const { data: res } = await this.$http.get(`permission/delete?id=${id}`)
